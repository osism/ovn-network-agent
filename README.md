...
- **Partial runtime reload** — `systemctl reload` (SIGHUP) applies the log level, reconcile interval, FRR prefix-list name, stale-chassis grace period, port forwards, shutdown and drain settings, and the OVN client certificate; every other key, including `ovn_ssl_ca`, needs a restart ([reload the configuration](https://osism.github.io/ovn-network-agent/guides/configuration#reload-the-configuration)).
